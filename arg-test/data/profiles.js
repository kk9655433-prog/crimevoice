...
function storyNow(){return typeof window.getArgNow==='function'?window.getArgNow():Date.now()}
function hasArrived(value,now=storyNow()){return now>=timeMs(value)}
...
function formatPublishedTime(item,now=storyNow()){
  ...
  if(typeof window.getArgDay==='function'&&dateInTaipei(published)===window.getArgDay())return '剛剛';
  return dateInTaipei(published);
}
