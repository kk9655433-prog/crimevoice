...
const ARG_DAY_START_HOUR=13;
function argDayStartMs(day){return new Date(`${day}T${String(ARG_DAY_START_HOUR).padStart(2,'0')}:00:00+08:00`).getTime()}
function getArgDay(){return state?.argDay||ARG_START_DAY}
function getArgNow(){return argDayStartMs(getArgDay())+Math.max(0,Date.now()-(state?.argDayEnteredAt||Date.now()))}
window.getArgDay=getArgDay;
window.getArgNow=getArgNow;
const state={
  ...
  argDay:saved.argDay||ARG_START_DAY,
  argDayEnteredAt:Number(saved.argDayEnteredAt)||Date.now(),
  declinedNextDay:!!saved.declinedNextDay,
  view:'news',
  previous:'news',
  lastHomeView:'news',
  homeScrolls:{}
...
function save(){
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      argDay:state.argDay,
      argDayEnteredAt:state.argDayEnteredAt,
      declinedNextDay:state.declinedNextDay
    })
  );
}
...
function effectivePublishedAt(item){
  if(item.publishedAt)return item.publishedAt;
  const match=String(item.time||'').match(/^(\d+)\s*(分鐘|小時|天)(?:前)?$/);
  if(!match)return '';
  const unitMs={分鐘:MINUTE,小時:HOUR,天:DAY}[match[2]];
  return new Date(argDayStartMs(getArgDay())-Number(match[1])*unitMs).toISOString();
}
function publishedTimeHTML(item){
  const publishedAt=effectivePublishedAt(item);
  const stamp=publishedAt?` data-published-at="${esc(publishedAt)}"`:'';
  return `<span class="dynamic-time"${stamp}>${esc(formatPublishedTime(publishedAt?{publishedAt}:item))}</span>`;
}
...
function releasedForArgDay(value){return dateInTaipei(value)<=getArgDay()}
function releasedScheduledNews(){return SCHEDULED_NEWS_POSTS.filter(post=>releasedForArgDay(post.publishedAt))}
function releasedProfileOnlyNews(){return PROFILE_ONLY_SCHEDULED_NEWS.filter(post=>releasedForArgDay(post.publishedAt))}
...
function openPost(id){
  ...
  if(!p)return;
  if([...SCHEDULED_NEWS_POSTS,...PROFILE_ONLY_SCHEDULED_NEWS].includes(p)&&!releasedForArgDay(p.publishedAt))return;
...
function nextArgDay(day=state.argDay){
  const next=new Date(`${day}T12:00:00+08:00`);next.setDate(next.getDate()+1);
  return dateInTaipei(next);
}
function updateNextDayButton(){
  const button=$('#nextDayButton');
  if(!button)return;
  button.classList.toggle('hidden',!state.declinedNextDay||state.argDay>=ARG_LAST_DAY);
  button.textContent=`進入 ${nextArgDay().slice(5).replace('-','/')} ›`;
}
function enterNextDay(){
  ...
  renderNews();renderNewsProfile();renderFeed();renderActivity(false);renderMessages();renderProgress();
  lastTimelineSignature='';refreshTimeline();toast(`已進入 ${state.argDay.slice(5).replace('-','/')}`);
}
...
$('#advanceDayButton').onclick=enterNextDay;
$('#stayTodayButton').onclick=()=>{state.declinedNextDay=true;save();$('#completionDialog').close();updateNextDayButton()};
$('#nextDayButton').onclick=enterNextDay;
