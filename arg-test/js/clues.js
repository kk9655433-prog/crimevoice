...
function currentTaipeiDay(){return typeof getArgDay==='function'?getArgDay():dateInTaipei(Date.now())}
...
function checkCompletion(){
  ...
  completionTimer=setTimeout(()=>{
    ...
    if(dialog&&!dialog.open){
      $('#completionDayLabel').textContent=today.slice(5).replace('-','/');
      $('#advanceDayButton').classList.toggle('hidden',today>=GAME_LAST_DAY);
      $('#stayTodayButton').textContent=today>=GAME_LAST_DAY?'繼續查閱':'否，繼續查閱';
      dialog.showModal();
    }
  },5000);
}
